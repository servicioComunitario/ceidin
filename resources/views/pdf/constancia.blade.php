...
<body>
	...
	<br>
	<p>
			  El suscrito Director(a) {{$director->datosBasico->nombre}}  {{$director->datosBasico->apellido}} con dedula n° {{$director->datosBasico->cedula}} del centro de Educacion Inicial "Desarrollo Integral del Niño", que funciona en Puerto Ordaz, Estado Bolivar, por medio de la presente, hace constar que el alumno(a):{{$alumno->datosBasico->nombre}} {{$alumno->datosBasico->apellido}} Sexo {{$alumno->datosBasico->sexo}}, de {{$edad}} años con cedula escolar n° {{$alumno->datosBasico->cedula}}, cursa el nivel Grupo de Educacion Inicial en la Seccion {{$inscripcion->docentePeriodo->seccion}} en esta Institucion, correspondiente al Año Escolar {{$inscripcion->periodo->fecha_inicio}}
	</p>
